...
/* ── Sidebar transparente, só azul nos detalhes ── */
...
[data-testid="stSidebar"] hr { border-color: rgba(59,91,219,0.12) !important; }
...
@media (max-width: 768px) {
    .block-container { padding: 1rem 0.5rem 2rem !important; }
...
@media (max-width: 480px) {
    .block-container { padding: 0.75rem 0.4rem 2rem !important; }
    .stTabs [data-baseweb="tab"] { padding: 5px 6px !important; font-size: 9px !important; }
}
</style>
  ...
  <svg id="bot" viewBox="0 0 400 500" xmlns="http://www.w3.org/2000/svg">
    ...
    <g class="jv-head">
      ...
      <g transform="translate(164,210)">
        <rect class="bar b1" x="0"  y="0" width="8" height="12" rx="2" fill="#3B5BDB" opacity="0.8"/>
        <rect class="bar b2" x="10" y="0" width="8" height="12" rx="2" fill="#3B5BDB" opacity="0.8"/>
        <rect class="bar b3" x="20" y="0" width="8" height="12" rx="2" fill="#D4880A" opacity="0.8"/>
        <rect class="bar b4" x="30" y="0" width="8" height="12" rx="2" fill="#3B5BDB" opacity="0.8"/>
        <rect class="bar b5" x="40" y="0" width="8" height="12" rx="2" fill="#D4880A" opacity="0.8"/>
        <rect class="bar b6" x="50" y="0" width="8" height="12" rx="2" fill="#3B5BDB" opacity="0.8"/>
        <rect class="bar b7" x="60" y="0" width="8" height="12" rx="2" fill="#3B5BDB" opacity="0.8"/>
      </g>
      <rect x="114" y="154" width="8" height="3" rx="1" fill="rgba(246,173,85,0.4)"/>
      <rect x="278" y="154" width="8" height="3" rx="1" fill="rgba(246,173,85,0.4)"/>
    </g>
    ...
    with col_form:
        ...
        <div style="padding:40px 0 24px 0;">
          ...
          <p style="color:#6B7280;font-size:14px;line-height:1.7;margin-top:12px;">
            ...
            Aguardando autenticação para acesso ao hub operacional.
          </p>
        </div>
        """, unsafe_allow_html=True)
...
with st.sidebar:
    ...
    if not df.empty:
        ...
        <div style="display:flex;flex-direction:column;gap:8px;">
          <div style="border:1px solid rgba(59,91,219,0.2);border-left:3px solid #3B5BDB;
          ...
          <div style="border:1px solid rgba(47,158,68,0.2);border-left:3px solid #2F9E44;
              border-radius:10px;padding:12px 14px;">
          ...
          <div style="border:1px solid rgba(212,136,10,0.2);border-left:3px solid #D4880A;
              border-radius:10px;padding:12px 14px;">
...
with tab2:
    ...
    if df.empty:
        ...
        with st.container():
            st.markdown("""
            <div style="background:#FFFFFF;border:1px solid #E2E4EA;border-radius:12px;
                padding:14px 18px;margin-bottom:18px;">
              <div style="font-family:'JetBrains Mono',monospace;font-size:9px;letter-spacing:2px;
                  color:#9CA3AF;margin-bottom:10px;">🔍 FILTROS</div>
            """, unsafe_allow_html=True)
        ...
        df_kanban = df.copy()
        if busca_nome:
            df_kanban = df_kanban[df_kanban["Projeto"].str.contains(busca_nome, case=False, na=False)]
        if data_ini_filtro:
            df_kanban = df_kanban[df_kanban["Prazo"].dt.date >= data_ini_filtro]
        if data_fim_filtro:
            df_kanban = df_kanban[df_kanban["Prazo"].dt.date <= data_fim_filtro]

        # Contador de resultados
        total_filtrado = len(df_kanban)
        if busca_nome or data_ini_filtro or data_fim_filtro:
            st.markdown(f"""
            <div style="font-size:11px;color:#6B7280;margin-bottom:12px;font-family:'JetBrains Mono',monospace;">
            {total_filtrado} projeto{'s' if total_filtrado != 1 else ''} encontrado{'s' if total_filtrado != 1 else ''}
            </div>
            """, unsafe_allow_html=True)
        ...
        for i, status in enumerate(STATUS_OPCOES):
            cor = STATUS_COLORS.get(status, "#3B5BDB")
            projetos = df_kanban[df_kanban["Status"] == status]
            count = len(projetos)
            ...
            with cols[i]:
                ...
                if projetos.empty:
                    st.markdown("""
                    <div style="font-size:11px;color:#CDD0DA;text-align:center;padding:12px 0;font-style:italic;">
                    sem resultados
                    </div>
                    """, unsafe_allow_html=True)

                for _, row in projetos.iterrows():
                    data_ini_str = pd.to_datetime(row.get("Data Inicial")).strftime("%d/%m/%Y") if pd.notna(row.get("Data Inicial")) else ""
                    ...
                    <div style="background:#FFFFFF;border:1px solid #E8EAEF;
                      ...
                      <div style="font-weight:600;font-size:13px;color:#1A1D2E;margin-bottom:6px;
                          line-height:1.3;">{row['Projeto']}</div>
                      <div style="font-size:11px;color:#9CA3AF;line-height:1.6;">
                        📅 <span style="color:#6B7280;">{data_ini_str}</span><br>
                        🎯 {foco}
                      </div>
                      <div style="margin-top:8px;display:flex;justify-content:space-between;align-items:center;">
                        <span style="font-size:10px;color:#9CA3AF;font-family:'JetBrains Mono',monospace;">prazo: {prazo_str}</span>
                        <span style="background:{badge_cor}18;color:{badge_cor};border:1px solid {badge_cor}33;
                            padding:2px 7px;border-radius:8px;font-size:9px;font-family:'JetBrains Mono',monospace;">
                          {dias}d
                        </span>
                      </div>
                    </div>
...
with tab5:
    ...
    # ── Motor de respostas estratégicas ──
    def gerar_resposta_local(pergunta: str, df: pd.DataFrame) -> str:
        p = pergunta.lower()

        if df.empty:
            return "⚠️ Nenhum dado carregado. Clique em **Sincronizar Dados** na barra lateral."
        ...
        backlog = len(df[df["Status"] == "A Iniciar"])
        futuros = len(df[df["Status"] == "Projetos Futuros"])
        reunioes = len(df[df["Status"] == "Reunião"])
        taxa = round(concluidos / total * 100, 1) if total > 0 else 0

        ativos = df[df["Status"].isin(["A Iniciar", "Em Andamento"])].copy()
        ativos["dias"] = (ativos["Prazo"] - agora).dt.days
        urgentes = ativos[ativos["dias"] < 7].sort_values("dias")
        criticos = ativos[ativos["dias"] < 14].sort_values("dias")
        proximos_30 = ativos[ativos["dias"] < 30].sort_values("dias")
        proximos_60 = ativos[ativos["dias"] < 60].sort_values("dias")
        atrasados = ativos[ativos["dias"] < 0].sort_values("dias")

        def lista(rows, col="Projeto", extra=None):
            out = []
            for _, r in rows.iterrows():
                ...
                if extra:
                    linha += f" — {extra(r)}"
                out.append(f"• {linha}")
            return "\n".join(out) if out else "_Nenhum_"

        # ── RISCO / ATRASO ──
        if any(x in p for x in ["risco", "atraso", "atrasado", "urgente", "crítico", "critico"]):
            if atrasados.shape[0] > 0:
                linhas = lista(atrasados, extra=lambda r: f"🔴 {abs(int(r['dias']))}d atrasado")
                return f"🚨 **{atrasados.shape[0]} projeto(s) já atrasado(s):**\n\n{linhas}\n\n⚡ Ação imediata requerida. Renegocie prazos ou realoque recursos agora."
            if urgentes.empty:
                return "✅ **Zero projetos em risco crítico** (< 7 dias).\n\nPortfólio com prazos controlados. Mantenha o ritmo."
            linhas = lista(urgentes, extra=lambda r: f"{int(r['dias'])}d restantes")
            return f"⚠️ **{urgentes.shape[0]} projeto(s) em zona crítica (< 7 dias):**\n\n{linhas}\n\n💡 Pare tudo e garanta a entrega desses antes de qualquer outra tarefa."

        # ── FOCO / PRIORIDADE / SEMANA ──
        elif any(x in p for x in ["foco", "semana", "energia", "prioridade", "começar", "iniciar hoje"]):
            if proximos_30.empty:
                return "📅 Nenhum prazo nos próximos 30 dias.\n\n💡 Ótimo momento para avançar no backlog ou antecipar projetos futuros."
            top = proximos_30.head(4)
            linhas = lista(top, extra=lambda r: f"{int(r['dias'])}d · {r['Status']}")
            return f"🎯 **Top prioridades para agora:**\n\n{linhas}\n\n💡 Ataque os menores prazos primeiro. Projetos 'Em Andamento' têm prioridade sobre 'A Iniciar'."

        # ── DIAGNÓSTICO GERAL ──
        elif any(x in p for x in ["diagnóstico", "diagnostico", "geral", "portfólio", "portfolio", "resumo", "overview", "situação", "situacao"]):
            saude = "🟢 Excelente" if taxa >= 70 else "🟡 Estável" if taxa >= 40 else "🔴 Crítico"
            return (
                f"📊 **Diagnóstico Completo do Portfólio:**\n\n"
                f"**Saúde geral:** {saude} ({taxa}% concluído)\n\n"
                f"• Total de projetos: **{total}**\n"
                f"• 🔥 Em execução: **{em_exec}**\n"
                f"• 📋 Backlog: **{backlog}**\n"
                f"• 🔭 Futuros: **{futuros}**\n"
                f"• 🤝 Reuniões: **{reunioes}**\n"
                f"• ✅ Concluídos: **{concluidos}**\n\n"
                f"**Alertas:** {urgentes.shape[0]} urgente(s) · {atrasados.shape[0]} atrasado(s)\n\n"
                f"{'💪 Continue assim, o pipeline está saudável.' if taxa >= 70 else '⚡ Foque em concluir o backlog antes de aceitar novos projetos.' if taxa >= 40 else '🚨 Taxa crítica. Revise prioridades e corte o que não é essencial.'}"
            )

        # ── ACELERAR / OPORTUNIDADES ──
        elif any(x in p for x in ["acelerar", "adiantar", "rápido", "velocidade", "oportunidade"]):
            livres = df[df["Status"] == "A Iniciar"].copy()
            livres["dias"] = (livres["Prazo"] - agora).dt.days
            livres = livres.sort_values("dias").head(4)
            if livres.empty:
                return "✅ Backlog vazio — excelente! Todos os projetos já foram iniciados.\n\n💡 Considere antecipar projetos futuros ou revisar os em andamento."
            linhas = lista(livres, extra=lambda r: f"prazo em {int(r['dias'])}d")
            return f"🚀 **Projetos prontos para iniciar agora:**\n\n{linhas}\n\n💡 Selecione o de maior impacto estratégico e mova para 'Em Andamento'."

        # ── GARGALOS / BLOQUEIOS ──
        elif any(x in p for x in ["gargalo", "problema", "bloqueio", "bloqueado", "travado", "parado"]):
            if criticos.empty:
                return "✅ Nenhum gargalo crítico detectado (< 14 dias).\n\n💡 Bom momento para revisão preventiva dos projetos em andamento."
            linhas = lista(criticos, extra=lambda r: f"{int(r['dias'])}d · {r['Status']}")
            return f"🔍 **Possíveis gargalos identificados:**\n\n{linhas}\n\n💡 Para cada um: identifique dependências, remova bloqueios e reforce equipe se necessário."

        # ── PRÓXIMOS 30 DIAS ──
        elif any(x in p for x in ["30 dias", "mês", "mes", "vencem", "vencimento", "prazo"]):
            if proximos_30.empty:
                return "📅 Nenhum projeto vence nos próximos 30 dias.\n\n💡 Use esse período para avançar no backlog e antecipar entregas futuras."
            linhas = lista(proximos_30, extra=lambda r: f"{int(r['dias'])}d · {r['Status']}")
            return f"📅 **{proximos_30.shape[0]} projeto(s) nos próximos 30 dias:**\n\n{linhas}"

        # ── PRÓXIMOS 60 DIAS ──
        elif any(x in p for x in ["60 dias", "dois meses", "bimestre", "trimestre"]):
            if proximos_60.empty:
                return "📅 Nenhum projeto vence nos próximos 60 dias."
            linhas = lista(proximos_60, extra=lambda r: f"{int(r['dias'])}d · {r['Status']}")
            return f"📅 **{proximos_60.shape[0]} projeto(s) nos próximos 60 dias:**\n\n{linhas}"

        # ── CARGA DE TRABALHO ──
        elif any(x in p for x in ["carga", "sobrecarga", "capacidade", "volume", "quantidade"]):
            carga = em_exec + backlog
            nivel = "🔴 Alta" if carga > 10 else "🟡 Moderada" if carga > 5 else "🟢 Controlada"
            return (
                f"⚖️ **Análise de Carga de Trabalho:**\n\n"
                f"• Carga ativa (execução + backlog): **{carga} projetos**\n"
                f"• Nível: {nivel}\n"
                f"• Em execução simultânea: **{em_exec}**\n"
                f"• Aguardando início: **{backlog}**\n\n"
                f"{'💡 Considere pausar novos projetos até reduzir o backlog.' if carga > 8 else '💡 Carga dentro do limite saudável. Você pode absorver mais projetos estratégicos.'}"
            )

        # ── PROJETOS CONCLUÍDOS / VITÓRIAS ──
        elif any(x in p for x in ["concluído", "concluido", "feito", "entregue", "vitória", "vitoria", "conquista"]):
            done = df[df["Status"] == "Concluído"]
            if done.empty:
                return "Nenhum projeto concluído ainda. Vamos nessa! 💪"
            linhas = "\n".join([f"✅ **{r['Projeto']}**" for _, r in done.iterrows()])
            return f"🏆 **{concluidos} projeto(s) entregue(s):**\n\n{linhas}\n\n🎯 Taxa de conclusão: **{taxa}%** · Continue esse ritmo!"

        # ── REUNIÕES ──
        elif any(x in p for x in ["reunião", "reuniao", "meeting", "agenda"]):
            meets = df[df["Status"] == "Reunião"]
            if meets.empty:
                return "📅 Nenhum projeto em fase de reunião no momento."
            linhas = "\n".join([f"🤝 **{r['Projeto']}**" for _, r in meets.iterrows()])
            return f"🤝 **{reunioes} projeto(s) em fase de reunião:**\n\n{linhas}\n\n💡 Garanta que cada reunião resulte em próximos passos concretos com dono e prazo definidos."

        # ── PROJETOS FUTUROS / PIPELINE ──
        elif any(x in p for x in ["futuro", "pipeline", "próximo projeto", "proximo projeto", "planejado"]):
            fut = df[df["Status"] == "Projetos Futuros"]
            if fut.empty:
                return "🔭 Nenhum projeto futuro mapeado.\n\n💡 Considere mapear seus próximos projetos para manter o pipeline aquecido."
            linhas = "\n".join([f"🔭 **{r['Projeto']}**" for _, r in fut.iterrows()])
            return f"🔭 **{futuros} projeto(s) no pipeline futuro:**\n\n{linhas}\n\n💡 Avalie quais podem ser antecipados dado o contexto atual."

        # ── RECOMENDAÇÃO ESTRATÉGICA ──
        elif any(x in p for x in ["recomendação", "recomendacao", "conselho", "estratégia", "estrategia", "o que fazer"]):
            rec = []
            if atrasados.shape[0] > 0:
                rec.append(f"🚨 **Emergência:** {atrasados.shape[0]} projeto(s) atrasado(s) — intervir agora.")
            if urgentes.shape[0] > 0:
                rec.append(f"⚠️ **Crítico:** {urgentes.shape[0]} projeto(s) vencem em < 7 dias — prioridade máxima.")
            if em_exec > 8:
                rec.append(f"⚖️ **Sobrecarga:** {em_exec} projetos em execução — considere pausar novos.")
            if backlog > 5:
                rec.append(f"📋 **Backlog alto:** {backlog} projetos aguardando — defina sequência de início.")
            if taxa < 40:
                rec.append(f"📉 **Taxa baixa ({taxa}%):** Revise o que está bloqueando entregas.")
            if not rec:
                rec.append(f"✅ Portfólio saudável com {taxa}% de conclusão. Mantenha o ritmo e antecipe o backlog.")
            return "🧠 **Recomendações Estratégicas J.A.R.V.I.S:**\n\n" + "\n\n".join(rec)

        # ── FALLBACK ──
        else:
            return (
                f"🤖 Analisando portfólio...\n\n"
                f"• **{em_exec}** projetos em execução · **{backlog}** no backlog\n"
                f"• **{urgentes.shape[0]}** urgente(s) · **{atrasados.shape[0]}** atrasado(s)\n"
                f"• Taxa de conclusão: **{taxa}%**\n\n"
                f"Selecione uma sugestão rápida ou tente: _'diagnóstico geral'_, _'o que fazer'_, _'carga de trabalho'_."
            )
    ...
    SUGESTOES = [
        ...
        ("⚖️", "Como está minha carga de trabalho?"),
        ("🧠", "Qual sua recomendação estratégica?"),
        ("🏆", "Quais projetos já foram concluídos?"),
        ("🔭", "O que está no pipeline futuro?"),
        ("🤝", "Quais projetos estão em reunião?"),
    ...
    if not st.session_state.chat_history:
        ...
        """, unsafe_allow_html=True)
        cols_s = st.columns(3)
        for i, (emoji, texto) in enumerate(SUGESTOES):
            with cols_s[i % 3]:
                if st.button(f"{emoji} {texto}", key=f"sug_{i}", use_container_width=True):
    ...
    for msg in st.session_state.chat_history:
        if msg["role"] == "user":
            ...
            conteudo = msg['content'].replace(chr(10), '<br>')
            # bold markdown -> html
            import re
            conteudo = re.sub(r'\*\*(.+?)\*\*', r'<strong>\1</strong>', conteudo)
            conteudo = re.sub(r'_(.+?)_', r'<em>\1</em>', conteudo)
            st.markdown(f"""
            <div style="display:flex;align-items:flex-start;gap:8px;margin:12px 0;">
              ...
              <div style="background:#FFFFFF;border:1px solid #E2E4EA;border-radius:3px 12px 12px 12px;
                  padding:14px 18px;max-width:85%;font-size:14px;line-height:1.8;color:#1A1D2E;
    ...
    if (enviar or pergunta_sugerida) and (user_input or pergunta_sugerida):
        ...
        resposta = gerar_resposta_local(query, df)
